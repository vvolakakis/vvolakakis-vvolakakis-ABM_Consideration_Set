...
rich-own [ utility-r employed ]
poor-own [ utility-p employed ]
jobs-own [ utility ]
patches-own [ quality price sd-dist ]
globals [counter view-mode min-poor-util max-poor-util min-rich-util max-rich-util
         unemployed-total employed-total rich-employed rich-unemployed poor-employed poor-unemployed
         economic-cycle boom-to-bust-prob bust-to-boom-prob boom-job-rate bust-job-rate economic-index]
...
to setup
  ...
  set unemployed-total 0
  set employed-total 0
  set rich-employed 0
  set rich-unemployed 0
  set poor-employed 0
  set poor-unemployed 0
  setup-jobs
  setup-patches
  setup-rich
  setup-poor
  ask patches [ update-patch-color ]
  set economic-cycle "boom"
  set boom-to-bust-prob 0.01 ;; probability of switching from boom to bust
  set bust-to-boom-prob 0.05 ;; probability of switching from bust to boom
  set boom-job-rate 7 ;; number of jobs created per step during boom
  set bust-job-rate 1 ;; number of jobs created per step during bust
  set economic-index 0 ;; initialize economic index
...
to setup-rich
  ...
  [
    ...
    raise-price
    raise-value
    set rich-unemployed rich-unemployed + 1
  ]
end
...
to setup-poor
  ...
  [
    ...
    decrease-price
    decrease-value
    set poor-unemployed poor-unemployed + 1
  ]
end
...
to go
  ...
  ;; Job creation and destruction based on economic cycle
  ifelse economic-cycle = "boom" [
    set counter ( counter + boom-job-rate )
  ] [
    set counter ( counter + bust-job-rate )
  ]
...
to evaluate-poor
  ...
  ifelse any? jobs in-radius 10 [
    if not employed [
      set employed true
      set employed-total employed-total + 1
      set poor-employed poor-employed + 1
      set poor-unemployed poor-unemployed - 1
    ]
  ] [
    if employed [
      set employed false
      set unemployed-total unemployed-total + 1
      set poor-employed poor-employed - 1
      set poor-unemployed poor-unemployed + 1
    ]
  ]
...
to evaluate-rich
  ...
  ifelse any? jobs in-radius 10 [
    if not employed [
      set employed true
      set employed-total employed-total + 1
      set rich-employed rich-employed + 1
      set rich-unemployed rich-unemployed - 1
    ]
  ] [
    if employed [
...
to-report patch-utility-for-poor
  ;report ( ( 1 / (sd-dist + 0.1) ) ^ ( 1 + price ) ) * ( quality ^ ( 1 - price) )
  report price
end
...
to-report patch-utility-for-rich
  report quality
  ;report ( ( 1 / (sd-dist + 0.1) ) ^ ( 1 - quality ) ) * ( quality ^ ( 1 + quality) )
end
...
to change-cycle
  ifelse economic-cycle = "boom" [
    if random-float 1 < boom-to-bust-prob [
      set economic-cycle "bust"
      show-message "The economy has shifted to a bust period."
    ]
  ] [
    if random-float 1 < bust-to-boom-prob [
      set economic-cycle "boom"
      show-message "The economy has shifted to a boom period."
    ]
  ]
end

to show-message [message]
  clear-output
  output-print message
end

;; Calculate the economic index based on the metrics
to calculate-economic-index
  let rich-employment-rate ifelse-value ((rich-employed + rich-unemployed) > 0) [rich-employed / (rich-employed + rich-unemployed)] [0]
  let poor-employment-rate ifelse-value ((poor-employed + poor-unemployed) > 0) [poor-employed / (poor-employed + poor-unemployed)] [0]
  let cycle-multiplier ifelse-value (economic-cycle = "boom") [1.2] [0.8]

  set economic-index ((rich-employment-rate + poor-employment-rate) / 2) * cycle-multiplier
end
