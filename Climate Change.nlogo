globals [
  ...
  ozone-layer  ;; thickness of the ozone layer
  ozone-layer-base ;; base level of the ozone layer
  tick-counter
  max-temperature
  max-CO2
  max-CH4
...
breed [CH4s CH4]     ;; packets of methane
breed [N2Os N2O]     ;; packets of nitrous oxide
...
to setup
  ...
  set-default-shape CH4s "hexagonal prism"
  set-default-shape N2Os "magnet"
  setup-world
  set temperature 12
  set ozone-layer 10 ;; Initial thickness of the ozone layer
  set ozone-layer-base sky-top - 9;; Base level of the ozone layer
  set tick-counter 0
  set max-temperature 50   ;; Assuming 50 is a critical high temperature
  set max-CO2 1000         ;; Maximum expected CO2 particles
  set max-CH4 500          ;; Maximum expected CH4 particles
  set max-N2O 300          ;; Maximum expected N2O particles
  reset-ticks
end
...
to go
  ...
  run-CH4   ;; moves CH4 molecules
  run-N2O   ;; moves N2O molecules
  update-ozone-layer
  clear-ozone-visualization
  display-ozone-layer
  set tick-counter tick-counter + 1
  if tick-counter mod 20 = 0 [
    probabilistic-gas-adjustments
  ]
  update-environmental-index
  tick
end
...
to add-CH4  ;; randomly adds 25 CH4 molecules to atmosphere
  let sky-height sky-top - earth-top
  create-CH4s 25 [
    set color orange
    ;; pick a random position in the sky area
...
to remove-CH4 ;; randomly remove 25 CH4 molecules
  repeat 25 [
    if any? CH4s [
      ask one-of CH4s [ die ]
    ]
...
to add-N2O  ;; randomly adds 25 N2O molecules to atmosphere
  let sky-height sky-top - earth-top
  create-N2Os 25 [
    set color violet
    ;; pick a random position in the sky area
...
to remove-N2O ;; randomly remove 25 N2O molecules
  repeat 25 [
    if any? N2Os [
      ask one-of N2Os [ die ]
    ]
...
to update-ozone-layer
  ;; Reduce ozone layer thickness based on greenhouse gas concentrations
  let total-greenhouse-gases (count CO2s + count CH4s + count N2Os)
  set ozone-layer (5 - 0.01 * total-greenhouse-gases)
end

to display-ozone-layer
  ask patches with [pycor > ozone-layer-base and pycor <= ozone-layer-base + ozone-layer] [
    set pcolor magenta
  ]
end

to clear-ozone-visualization
  ask patches with [pcolor = magenta and pycor > ozone-layer-base and pycor <= ozone-layer-base + 10] [
    set pcolor scale-color blue pycor -20 20
  ]
end

to probabilistic-gas-adjustments
  let add-CO2-prob 0.9
  let remove-CO2-prob 0.1
  let add-CH4-prob 0.8
  let remove-CH4-prob 0.05
  let add-N2O-prob 0.5
  let remove-N2O-prob 0.02

  ;; Adjust CO2
  if random-float 1 < add-CO2-prob [
    add-CO2
  ]
  if random-float 1 < remove-CO2-prob [
    remove-CO2
  ]

  ;; Adjust CH4
  if random-float 1 < add-CH4-prob [
    add-CH4
  ]
  if random-float 1 < remove-CH4-prob [
    remove-CH4
  ]

  ;; Adjust N2O
  if random-float 1 < add-N2O-prob [
    add-N2O
  ]
  if random-float 1 < remove-N2O-prob [
    remove-N2O
  ]
end

to update-environmental-index
  let normalized-temperature temperature / max-temperature
  let normalized-CO2 (count CO2s) / max-CO2
  let normalized-CH4 (count CH4s) / max-CH4
  let normalized-N2O (count N2Os) / max-N2O

  let weight-CO2 0.2
  let weight-CH4 0.3
  let weight-N2O 0.5

  let weighted-CO2 normalized-CO2 * weight-CO2
  let weighted-CH4 normalized-CH4 * weight-CH4
  let weighted-N2O normalized-N2O * weight-N2O

  set environmental-index (normalized-temperature + weighted-CO2 + weighted-CH4 + weighted-N2O) / (1 + weight-CO2 + weight-CH4 + weight-N2O)
end
